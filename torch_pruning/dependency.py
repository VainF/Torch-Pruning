...
class PruningPlan(list):
    def __init__(self, dry_run=False, module_to_name=None):
        """ A pruning plan consists of several dependencies. 
    ...
    def append(self, x):
        assert len(x)==2 and isinstance(x[0], Dependency) and isinstance( x[1], (list, tuple) ), "a pruning plan must consists of a dependency and a index list"
        if x not in self:
    ...
    def __str__(self):
        ...
        for dep, idxs in self:
            ...
            if self._module_to_name is not None:
                str_result+="[ %s on %s (%s), Indexs=%s, NumPruned=%d]\n"%(dep.handler.__name__, self._get_module_name(dep.target), dep.target, idxs, n_pruned)
            else:
                str_result+="[ %s on %s (%s), Indexs=%s, NumPruned=%d]\n"%(dep.handler.__name__, dep.target, dep.target, idxs, n_pruned)
...
class Dependency(object):
    ...
    def __call__(self, idxs, dry_run=False):
        ...
        if not dry_run and isinstance(self.target, _ConcatOp): # update offset after pruning
...
class DependencyGraph(object):
    ...
    ALL_PENDING_GRAD_FN_TYPE = PENDING_GRAD_FN_TYPE + PENDING_ELEMENTWISE_GRAD_FN_TYPE + PENDING_CONCAT_GRAD_FN_TYPE
    ...
    def __init__(self, model: nn.Module, fake_input: torch.Tensor, ignore_conv_fc_dep=False):
        self.dependencies = {}
    ...
    def get_pruning_plan(self, module, pruning_fn, idxs):
        ...
        parameters:
            ...
            idxs (list): the pruning indexs in module.
        """

        visited_module = set()
        triggered_deps = PruningPlan(module_to_name = self.module_to_name)
        triggered_deps.append( ( Dependency(None, pruning_fn, module ), idxs ) ) # manually specified pruning operation
    ...
    def add_dependency(self, source_module: nn.Module, target_module: nn.Module, dep: Dependency):
    ...
    def _get_dependency(self, source_fn_name: str, target_fn_name: str, reverse=False):
        if not reverse:
    ...
    def build_dependency(self, model: nn.Module, fake_input: torch.Tensor):
        """build pruning dependencies on the model. A fake input is required to obatin the dynamic graph.
        ...
        def record_grad_fn(module, input, output):
            ...
            if isinstance(module, TORCH_LINEAR) and fc_after_conv==True:
                fc_after_conv = False
                if reduce( mul, extractor_feature_shape )==reduce( mul, input[0].shape ): # reshape
                    self.conv_fc_stride[module] = reduce( mul, extractor_feature_shape[2:] )
                elif reduce( mul, extractor_feature_shape[:2] )==reduce( mul, input[0].shape ): # global pooling
    ...
    def _traverse_graph(self, begin_nodes):
        ...
        def _cal_current_channels_and_index_offset(n, previous_ch):
        ...
        def _collect_pending_nodes_and_model_info(node):
            ...
            if grad_fn_name=='ElementWiseOpBackward' and node not in self.grad_fn_to_module.keys():
                self.grad_fn_to_module[node] = _ElementWiseOp() # create a Dummy Module
            ...
            cur_channels = _cal_current_channels_and_index_offset(node, previous_channels)

            #if node in self.grad_fn_to_module.keys():
            #    print( self.grad_fn_to_module[node] , cur_channels, previous_channels)
        ...
        def _recursively_build_dependencies(node, path_id):
            if begin_node != node:
            ...
            if hasattr(node, 'next_functions'):
                ...
                for u in node.next_functions:
                    #print(u)
                    if u[0] is not None:
                        _recursively_build_dependencies(u[0], path_id)
                        path_id+=1
        for begin_node in pending_nodes:
            _recursively_build_dependencies( begin_node, 0 )

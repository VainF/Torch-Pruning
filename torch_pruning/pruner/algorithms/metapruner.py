...
class MetaPruner:
    ...
    def __init__(
        ...
        if ignored_layers is not None:
            warnings.warn("ignored_layers is deprecated in v1.4.0. Please use ignored_layer_outputs or ignored_layer_inputs instead.")
            ignored_layer_outputs = ignored_layers
        ...
        def _get_submodules(layer_list):
            expanded = []
            for layer in layer_list:
                if isinstance(layer, nn.Module):
                    expanded.extend(list(layer.modules()))
            return expanded
        self.ignored_layer_outputs = _get_submodules(ignored_layer_outputs)
        self.ignored_layer_inputs = _get_submodules(ignored_layer_inputs)
        self.ignored_params = ignored_parameters
        ...
        if self.global_pruning:
            ...
            for group in self.DG.get_all_groups(ignored_layer_inputs=self.ignored_layer_inputs, ignored_layer_outputs=self.ignored_layer_outputs, target_layers=self.target_layers, target_layer_types=self.target_layer_types):
    ...
    def _downstream_node_as_root_if_attention(self, group):
        ...
        for _dep, _idxs in group:
            if _dep.source.module in self.num_heads and self.DG.is_out_channel_pruning_fn(_dep.handler):
                is_attention = True
            if isinstance(_dep.target.module, tuple(self.target_layer_types)) and self.DG.is_in_channel_pruning_fn(_dep.handler):
                downstream_dep = _dep

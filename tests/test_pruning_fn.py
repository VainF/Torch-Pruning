...
print(
    "%d weights were actually masked"
    % ((model.features[0].weight_mask.numpy() != 0).sum())
)
print(
    "mask1 | mask2 == weight_mask: ",
    np.alltrue(np.logical_or(mask1, mask2) == model.features[0].weight_mask.numpy()),
)
...
conv1_output_target = F.conv2d(
    random_inputs, masked_weight, bias=model.features[0].bias, stride=4, padding=2
)

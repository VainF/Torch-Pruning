...
Dependency exists between parameters/layers, which forces a group of parameters to be pruned simultaneouly. This works provides an automatical mechanism to group coupled parameters, so that they can be correctly removed during pruning. To be exact, Torch-Pruning will forward your model with a fake inputs and trace the computational graph just like ``torch.jit``. A dependency graph will be established to record the coupling between layers. When you prune a layer, Torch-pruning will collect all affected layers and return a `PruningGroup`. All pruning indices will be automatically transformed if there are operations like ``torch.split`` or ``torch.cat``. We illustrate some dependencies in modern nerual networks bellow:

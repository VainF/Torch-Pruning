...
#### Modification
Few functions need modification to prevent performance loss during model saving.

###### 1. ```train``` in class ```YOLO```
This function creates new YOLO trainer when called. YOLO trainer loads model based on config file and reassign it to current model, which should be avoided for pruning.

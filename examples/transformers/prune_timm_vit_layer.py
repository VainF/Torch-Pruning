...
def main():
    ...
    if args.pruning_type == 'random':
        ...
        layer_imp = tp.importance.RandomImportance(normalizer=None)
    elif args.pruning_type == 'taylor':
        imp = tp.importance.GroupTaylorImportance()
        layer_imp = tp.importance.GroupTaylorImportance(normalizer=None)
    elif args.pruning_type == 'l2':
        imp = tp.importance.GroupNormImportance(p=2)
        layer_imp = tp.importance.GroupNormImportance(p=2, normalizer=None)
    elif args.pruning_type == 'l1':
        imp = tp.importance.GroupNormImportance(p=1)
        layer_imp = tp.importance.GroupNormImportance(p=1, normalizer=None)
    elif args.pruning_type == 'hessian':
        imp = tp.importance.GroupOBDImportance()
        layer_imp = tp.importance.GroupOBDImportance(normalizer=None)
    ...
    if isinstance(imp, (tp.importance.GroupTaylorImportance, tp.importance.GroupOBDImportance)):
        ...
        for k, (imgs, lbls) in enumerate(train_loader):
            ...
            if isinstance(imp, tp.importance.GroupOBDImportance):
                loss = torch.nn.functional.cross_entropy(output, lbls, reduction='none') # reduction='none' to compute the loss for each sample
                for l in loss:
                    model.zero_grad() # clear the previous gradients
                    l.backward(retain_graph=True) # compute the gradients
                    imp.accumulate_grad(model) # accumulate the gradients
            elif isinstance(imp, tp.importance.GroupTaylorImportance):
                loss = torch.nn.functional.cross_entropy(output, lbls) # simply compute the loss
                loss.backward() # compute and accumulate the gradients
    ...
    print(model)
    def replace_with(original):
        return torch.nn.Identity()
    pruner.prune_layers(model.blocks, pruning_ratio=0.5, importance=layer_imp, replace_with=replace_with)
    print(model)

...
def main_worker(gpu, ngpus_per_node, args):
    ...
    evaluator = engine.evaluator.classification_evaluator(val_loader)
...
def train(train_loader, model, criterion, optimizer, args):
    global best_acc1
    loss_metric = engine.metrics.RunningLoss(nn.CrossEntropyLoss(reduction='sum'))
    acc_metric = engine.metrics.TopkAccuracy(topk=(1,5))
    model.train()
